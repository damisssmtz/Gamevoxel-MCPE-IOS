...
//		GLState::setAlphaTestEnabled(false); // vanilla pe disables alpha test here, i renable it so grass side textures dont become opaque - shredder
		...
		//} else
		{
			// Water Depth Buffer Setup
			GLint defaultFBO;
			glGetIntegerv(GL_FRAMEBUFFER_BINDING, &defaultFBO);

			// [OPTIMIZATION] Disabled expensive depth FBO blit.
			// Resolving the depth buffer every frame causes massive pipeline stalls on mobile/integrated GPUs,
			// dropping FPS to 20. Water shader now uses procedural depth.
			// if (waterDepthFBO == 0 || fboWidth != mc->width || fboHeight != mc->height) {
			//    ... setup omitted for performance ...
			// }
			// glBindFramebuffer(GL_READ_FRAMEBUFFER, defaultFBO);
			// glBindFramebuffer(GL_DRAW_FRAMEBUFFER, waterDepthFBO);
			// glBlitFramebuffer(0, 0, mc->width, mc->height, 0, 0, mc->width, mc->height, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
			...
			// Custom AAA Water Shader
			TIMER_POP_PUSH("terrain-water-shader");
			bool useWaterShader = mc->options.getBooleanValue(OPTIONS_WATER_SHADER);
			if (useWaterShader) {
				WaterShader::instance.bind();
				GLState::useWaterShader = true;
				
				// Setup uniforms for water shader
				WaterShader::setupMVP(GLState::getMVP());
				WaterShader::instance.setUniformMatrix4f("u_ModelView", GLState::getModelView());
				WaterShader::instance.setUniformMatrix4f("u_Projection", GLState::getProjection());
				WaterShader::instance.setUniform1i("u_UseFog", 0);
				
				// Bind Depth Texture
				glActiveTexture(GL_TEXTURE1);
				glBindTexture(GL_TEXTURE_2D, waterDepthTex);
				WaterShader::instance.setUniform1i("depth_texture", 1);
				
				// Calculate time in seconds for the shader
				float timeInSeconds = ((float)mc->level->getTime() + a) / 20.0f;
				WaterShader::instance.setUniform1f("u_Time", timeInSeconds);
			}
			...
			if (useWaterShader) {
				...
				GLState::useWaterShader = false;
				WaterShader::instance.unbind();
				
				// Restore TerrainShader for anything else that expects it
...
void GameRenderer::saveMatrices()
{
	...
	GLState::getFloatv(GL_PROJECTION_MATRIX, lastProjMatrix);
	GLState::getFloatv(GL_MODELVIEW_MATRIX, lastModelMatrix);
}
