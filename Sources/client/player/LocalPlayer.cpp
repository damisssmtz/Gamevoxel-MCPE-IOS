...
void LocalPlayer::aiStep() {
	...
	// Sprint: detect W double-tap
	{
		...
		if (forwardHeld && !prevForwardHeld && minecraft->options.getBooleanValue(OPTIONS_ALLOW_SPRINT)) {
			...
			if (sprintDoubleTapTimer > 0 && foodData.getFoodLevel() > 6)
				sprinting = true;
			else
				sprintDoubleTapTimer = 7;
		}
		if (!forwardHeld || foodData.getFoodLevel() <= 6) {
			sprinting = false;
		}
	...
	if (sprinting && (input->xa != 0 || input->ya != 0)) {
		foodData.addExhaustion(0.005f);
	}
	if (!wasJumping && input->jumping) {
		foodData.addExhaustion(sprinting ? 0.08f : 0.02f);
	}
