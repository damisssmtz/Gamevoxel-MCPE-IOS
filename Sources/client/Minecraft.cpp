	...
	while (Mouse::next()) {
		...
		if (e.action == MouseAction::ACTION_WHEEL) {
			...
			if (screen && (dynamic_cast<ChatScreen*>(screen) || dynamic_cast<ConsoleScreen*>(screen))) {
				...
				if (numSlots > 0) {
					int slot = (v->selected - e.dy % numSlots + numSlots) % numSlots;
					v->selectSlot(slot);
				}
			}
		}
	...
	while (Keyboard::next()) {
		...
		if (isPressed) {
			...
			if (key >= '0' && key <= '9') {
				...
				int maxSelectable = useTouchscreen() ? (gui.getNumSlots() - 1) : gui.getNumSlots();
				if (slot >= 0 && slot < maxSelectable)
...
#ifndef STANDALONE_SERVER
	const char* langCodes[] = {"en_US", "es_ES"};
	int langIdx = options.getIntValue(OPTIONS_LANGUAGE);
	if (langIdx >= 0 && langIdx < 2) {
		I18n::loadLanguage(platform(), langCodes[langIdx]);
	} else {
...
void Minecraft::pushScreen(Screen* newScreen) {
    if (this->screen) {
        ...
        this->screen = NULL; // prevent deletion
    }
    setScreen(newScreen);
}
...
void Minecraft::popScreen() {
    if (screenStack.empty()) {
        ...
        Screen* oldScreen = screenStack.back();
