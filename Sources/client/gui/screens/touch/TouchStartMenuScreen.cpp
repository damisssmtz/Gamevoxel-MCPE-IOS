...
void StartMenuScreen::setupPositions() {
	...
	// Estimate logo bounds for Touch UI
	// Logo height is scale * data->h, where scale = 2 * wh / data->w
	// and wh = 0.5 * Min(width/2, data->w/2). Assuming data->w ~ 274, data->h ~ 66
	float logoH = (std::min)((float)width / 2.0f, 137.0f) * (66.0f / 137.0f);
	int logoBottom = 4 + (int)logoH;
	
	// Center vertically in the remaining space
	int yBase = logoBottom + (height - logoBottom - bHost.height) / 2;
	if (yBase < logoBottom + 5) {
		yBase = logoBottom + 5;
	}
	...
	// Add safe area margin for the quit icon (top-right)
	int safeMarginX = (width > 600) ? 20 : 6;
	bQuit.x = width - bQuit.width - safeMarginX;
	bQuit.y = 6;

	copyrightPosX = width - minecraft->font->width(copyright) - safeMarginX;
...
void StartMenuScreen::render( int xm, int ym, float a )
{
	...
	if (data) {
		...
		drawString(font, "Cubic/Minecraft", 12, height - 10, 0xffcccccc);
		//patch->draw(t, 0, 20);
	}
...
void StartMenuScreen::mouseClicked(int x, int y, int buttonNum) {
	const int logoX = 2;
	const int logoW = 8 + 2 + font->width("Cubic/Minecraft");
	const int logoY = height - 10;
	const int logoH = 10;
	if (x >= logoX && x <= logoX + logoW && y >= logoY && y <= logoY + logoH)
		minecraft->platform()->openURL("https://github.com/Cubic/Minecraft");
	else
