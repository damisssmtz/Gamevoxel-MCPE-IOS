...
public:
	...
	void renderHearts();
	void renderHunger();
	void renderExperienceBar(Font* font, int screenWidth, int screenHeight);
	void renderModernStatusCard(Font* font, int screenWidth, int screenHeight);
	void addFloatingPopup(const std::string& text, int color, float x, float y);
	void updateFloatingPopups(float dt);
	void renderFloatingPopups(Font* font);
...
private:
	...
	int lastTrackedFood;
	float heartBounceImpact;
	int lastTrackedLevel;
	float levelUpFlash;
};
