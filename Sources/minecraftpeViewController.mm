...
#import "Sources/platform/input/Multitouch.h"
#import "Sources/platform/input/Mouse.h"
#import "Sources/client/OptionStrings.h"
...
#import "Sources/InAppSettingsKit/Models/IASKSettingsReader.h"

#import "Sources/InAppSettingsKit/Views/IASKSwitch.h"
#import "Sources/InAppSettingsKit/Views/IASKPSToggleSwitchSpecifierViewCell.h"
...
- (void)viewWillLayoutSubviews
{
    ...
    CGRect oldFrame = self.glView.frame;
    CGRect newFrame = self.view.bounds;
    
    if (!CGRectEqualToRect(oldFrame, newFrame)) {
        self.glView.frame = newFrame;
        
        if (_app && _app->isInited()) {
            CGFloat width  = MAX(newFrame.size.width, newFrame.size.height) * viewScale;
            CGFloat height = MIN(newFrame.size.width, newFrame.size.height) * viewScale;
...
- (BOOL) initView
{
    viewScale = self.glView->viewScale;
    ...
    CGRect screen = self.glView.frame;
...
//    CGFloat width  = self.glView->framebufferWidth;
//    CGFloat height = self.glView->framebufferHeight;
...
//        NSLog(@"touch-began: %d %p\n", pointerId, touch);
        if (pointerId >= 0) {
            CGPoint touchLocation = [touch locationInView:self.glView];
...
- (void)showDialog_SetUsername
{
    if (!_dialog) {
        ...
        NSString* currentUsername = [NSString stringWithUTF8String:((Minecraft*)_app)->options.getStringValue(OPTIONS_USERNAME).c_str()];
        if (dlg->_textName) {
