...
- (void) initAudio {
    ...
    // Use Playback so sounds play even when the Ring/Silent switch is on.
    // Change to AVAudioSessionCategoryAmbient if you prefer respecting the silent switch.
    audioSessionSoundCategory = AVAudioSessionCategoryPlayback;

    BOOL ok = [audioSession setCategory:audioSessionSoundCategory
                            withOptions:AVAudioSessionCategoryOptionMixWithOthers
                                  error:&audioSessionError];
    if (!ok || audioSessionError) {
        NSLog(@"[Audio] ERROR setting category: %@", audioSessionError);
        audioSessionError = nil;
        // Fallback to Ambient
    ...
    audioSessionError = nil;
    ok = [audioSession setActive:YES error:&audioSessionError];
    if (!ok || audioSessionError) {
        NSLog(@"[Audio] ERROR activating session: %@", audioSessionError);
    } else {
        NSLog(@"[Audio] AVAudioSession active (category = %@)", audioSessionSoundCategory);
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    self.viewController = [[minecraftpeViewController alloc] init];
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    NSLog(@"become-active: %@\n", [NSThread currentThread]);
#endif
    // Re-activate audio session after interruptions / background
