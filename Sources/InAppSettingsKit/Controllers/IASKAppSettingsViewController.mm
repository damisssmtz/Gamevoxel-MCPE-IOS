...
- (CGFloat)tableView:(UITableView*)tableView heightForHeaderInSection:(NSInteger)section {
    ...
    if ((title = [self tableView:tableView titleForHeaderInSection:section])) {
        ...
        CGSize maxSize = CGSizeMake(tableView.frame.size.width - 2*kIASKHorizontalPaddingGroupTitles, CGFLOAT_MAX);
        CGRect textRect = [title boundingRectWithSize:maxSize
                                              options:NSStringDrawingUsesLineFragmentOrigin
                                           attributes:@{NSFontAttributeName: font}
                                              context:nil];
        
        return textRect.size.height + kIASKVerticalPaddingGroupTitles;
    }
    return 0;
}
